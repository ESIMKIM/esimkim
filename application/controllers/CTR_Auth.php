...
class CTR_Auth extends CI_Controller
{
	...
	{
		// $token = bin2hex(random_bytes(16));
		$this->session->unset_userdata('breach_menu');
			...
			if (!empty($users)) {
				foreach ($users as $data) {
					if (password_verify($password, $data->sandi)) {
						...
						$this->session->set_userdata('role_id', $data->role_id);
						$this->session->set_userdata('nip', $data->nip);
						$this->session->set_userdata('dep_name', $data->department_name);
						$this->session->set_userdata('bagian_name', $data->section_name);
						...
						$array = array(
							'error'   => true,
							'pesan' => "<p> Username atau sandi salah <p>"
						);
					}
				}
			} else {
