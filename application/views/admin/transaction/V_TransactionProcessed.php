<div id="mGagalSimpanKurangApproval" class="modal fade">
    <div class="modal-dialog modal-confirm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <span>Ada Item yang belum diAlokasi jumlah Pengirimannya</span>
            </div>
        </div>
    </div>
</div>

<div id="mGagalSimpanSubmit" class="modal fade">
    <div class="modal-dialog modal-confirm">
        <div class="modal-content">
            <div class="modal-header">
                <div class="icon-box" style="background-color: red;">
            ...
            <div class="modal-body">
                ...
                <!-- <p class="text-center"></p> -->
                <span>Harap Hubungi Develpoer</span>
            </div>
        </div>
    </div>
</div>
                        ...
                        <table id="mydata" class="table">
                            <thead>
                                <tr class="text-nowrap" style="text-align:center;">
                                    ...
                                    <th style="text-align:center;">Alokasi Bulan ini</th>
                            ...
                            <tbody id="show_data">
                                ...
                                foreach ($item as $datas) : ?>
                                    <tr>
                                        <input hidden style=" width: 100px;" type="text" id="idi_<?php echo $i ?>" name="idi" class="form-control" value="<?= $datas->th_id; ?>" />
                                        <input hidden style="width: 100px;" type="text" id="prod_<?php echo $i ?>" name="prod" class="form-control" value="<?= $datas->products_id; ?>" />
                                        ...
                                        <td style="text-align:center;"><?= $datas->alokasi; ?></td>


                                        <?php if ($item[0]->is_done == 1) : ?>
                                            <td style="text-align:center;"><input disabled value="<?= $datas->approval; ?>" type="text" id="setujui_<?php echo $i ?>" name="setujui" style="width: 50px; text-align: center; background-color: lightgray;" onchange="updatesapproval(<?php echo $i ?>)" />
                                            </td>
                                        <?php else : ?>
                                            <td style="text-align:center;"><input value="<?= $datas->approval; ?>" type="text" id="setujui_<?php echo $i ?>" name="setujui" style="width: 50px; text-align: center;" onchange="updatesapproval(<?php echo $i ?>)" />
                                            </td>
                                        <?php endif; ?>


                                        <td style="text-align:center;"><?= $datas->sisa; ?></td>
                                        <!-- <?php if ($item[0]->is_done == 1) : ?>
...
<script type="text/javascript">
    ...
    function update_isDone(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.errorqty) {
                    $('#mGagalSimpanKurangApproval').modal('show');

                } else if (data.errorqty) {
                    console.log("error : " + data.pesan);
                    $('#mGagalSimpanSubmit').modal('show');
                }
